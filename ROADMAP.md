...
- **Priority:** Medium
- **Tasks:**
  - [x] **Data Structure Update:** Update `FileItem` struct in `file_tree.rs` to support `git_status` (Modified, Added, Ignored), `diagnostic_status` (Error, Warning), and "folded" directory names (e.g., "src/components").
  - [x] **Component Refactor (`render_file_item`):** Re-implement the rendering logic to match Zed's `ListItem` layout:
    - [x] Use `margin-left` for indentation instead of spacer divs.
    - [x] Implement absolute positioning for disclosure chevrons (arrows).
    - [x] Support "folded" directory rendering (single line for single-child directories).
  - [x] **Theme Integration:** Apply authentic Zed theme keys:
    - [x] `git.status.modified`, `git.status.created`, `git.status.ignored` for filename colors.
...
- **Brittle Layout Constraints:** The "Editor Preview" pane currently relies on fixed-height constraints (e.g., 600px on the main container) to force internal components like the editor area and terminal to share space correctly and enable scrolling. Specifically, the terminal's height is particularly resistant to dynamic resizing and often requires a fixed pixel height, contributing to the layout's brittleness. This is a workaround for complex flexbox behavior in GPUI where nested `flex-grow` containers can expand indefinitely. While functional for this fixed-size demo, a robust implementation would require a complete refactor of the flex hierarchy to support fully responsive resizing.
