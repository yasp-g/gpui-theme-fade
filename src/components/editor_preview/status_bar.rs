...
pub fn render_status_bar(theme: &InterpolatableTheme) -> impl IntoElement {
    ...
    let border_variant = theme
        .0
        .get("border.variant")
        .map_or(hsla(0., 0., 0.5, 0.2), |c| c.hsla);
    ...
    div()
        ...
        .child(
            ...
            div()
                ...
                .child(div().w_px().h_4().bg(border_variant)) // Separator
                ...
                .child(render_diagnostics(theme)),
        )
        .child(
            // Right Tools
            div()
...
fn render_icon_button(
    ...
    render_button_container(
        ...
        gpui::svg()
            .path(path)
            .size(Rems(1.0))
            .text_color(if active { icon_accent } else { icon_muted }),
    )
}
...
fn render_diagnostics(theme: &InterpolatableTheme) -> impl IntoElement {
    ...
    let warning_color = theme
        .0
        .get("warning")
        .map_or(hsla(0.1, 1.0, 0.5, 1.), |c| c.hsla);
    let text_color = theme
        .0
        .get("status_bar.foreground")
        .map_or(hsla(0., 0., 0.6, 1.), |c| c.hsla);
